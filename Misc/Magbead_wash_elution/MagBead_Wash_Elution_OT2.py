...
######################### Load Labware #####################################
    ...
    elution_buffer = Purification.load_labware('nest_96_wellplate_2ml_deep', 3, label = 'Elution Buffer')
    waste = Purification.load_labware('nest_96_wellplate_2ml_deep', 9, label= 'Waste')

    tiprack_1 = Purification.load_labware('opentrons_96_tiprack_300ul', 4, label='300uL Tips')
    pipette_P300 = Purification.load_instrument('p300_multi_gen2', mount = 'left', tip_racks = [tiprack_1])
...
############################# Protocol #####################################
    ...
    for column_number, column_name in dictonary.items():
        ...
        locations = [wash_buffer, wash_buffer, wash_buffer, elution_buffer]
